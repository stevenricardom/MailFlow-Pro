// screens/edit_filter_screen.dart
...
import 'review_filter_screen.dart';
...
class _EditFilterScreenState extends State<EditFilterScreen> {
  ...
  void initState() {
    super.initState();
    // Precargar los datos del filtro existente
    ...
    _criteriaController = TextEditingController(text: widget.filter.criteria);
    _filterType = widget.filter.type;
    _autoReply = widget.filter.autoReply;
    _active = widget.filter.active;
    _caseSensitive = widget.filter.caseSensitive;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Editar Filtro', style: AppTextStyles.h2),
      ),
      body: Padding(
        ...
        MaterialPageRoute(
          builder: (context) => ReviewFilterScreen(
            ...
            onConfirm: (confirmedFilter) {
              widget.onFilterUpdated(confirmedFilter);
